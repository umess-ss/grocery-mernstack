...
const Navbar = () => {
  ...
  return (
    ...
    <div className="hidden sm:flex items-center gap-8">
        ...
        <div className="hidden lg:flex items-center text-sm gap-2 border border-gray-300 px-3 rounded-full">
            <input onChange={(e)=>setSearchQuery(e.target.value)} className="py-1.5 w-full bg-transparent outline-hidden placeholder-gray-500" type="text" placeholder="Search products" />
            <img src={assets.search_icon} alt="searching img" className="w-4 h-4"/>
        </div>
        ...
        (<div className='relative group'>
            ...
            <ul className='hidden group-hover:block absolute top-10 bg-white shadow-sm border border-gray-200 py-2.5 w-30 rounded-md text-sm z-40'>
 ...
 {open &&(  
  <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] z-50 left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
        ...
        {user &&
        <NavLink to="/contacts" onClick={()=>setOpen(false)}>My Orders</NavLink>
        }
